      ...
      {computedValue && (
        <div className="aspect-w-2 aspect-h-1">
          ...
          <div className='opacity-0 bg-black hover:bg-opacity-50 hover:opacity-100 flex items-center justify-center space-x-2'>
            <ImagePicker imageType='article-image' onSelected={triggerChange}>
              <button className='btn btn-sm btn-primary'>Ubah</button>
            </ImagePicker>
            <button className='btn btn-sm btn-error' onClick={() => triggerChange(null)}>Hapus</button>
          </div>
        </div>
      )}
      {!computedValue && (
        <ImagePicker imageType='article-image' onSelected={triggerChange}>
          <button className="block w-full aspect-w-2 aspect-h-1 rounded-lg border-2 border-dashed border-slate-400 text-slate-400 bg-white">
